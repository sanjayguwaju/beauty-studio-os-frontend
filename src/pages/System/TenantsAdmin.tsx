...
export default function TenantsAdmin() {
  ...
  const handleImpersonate = async (tenantId: string) => {
    try {
      const reason = prompt("Please enter a reason for impersonating this tenant (will be audit logged):");
      if (!reason) return;
      const res = await api.post("/system/impersonate", { tenantId, reason });
      if (res.data.success) {
        const original = localStorage.getItem("token");
        if (original) localStorage.setItem("token_original", original);
        localStorage.setItem("token", res.data.data.token);
        toast.success("Impersonating tenant — reloading...");
                ...
                >
                  ...
                  <DropdownMenu.Item asChild>
                    <button
                      ...
                      className="flex w-full items-center gap-2 px-3 py-2 text-sm text-purple-700 hover:bg-purple-50 rounded-lg dark:text-purple-400 dark:hover:bg-purple-900/30 transition-colors text-left"
                    >
                      <UserCircle className="w-4 h-4" /> Impersonate
                    </button>
                  </DropdownMenu.Item>
